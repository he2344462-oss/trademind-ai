...
func TestSprint5MigrationIsIdempotent(t *testing.T) {
	...
	require.True(t, harness.DB.Migrator().HasTable(&productflow.ListingContentQualityReview{}))
	require.True(t, harness.DB.Migrator().HasTable(&productflow.ListingOperationBatch{}))
	require.True(t, harness.DB.Migrator().HasTable(&productflow.ListingOperationBatchItem{}))
