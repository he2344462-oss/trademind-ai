...
export type SelectionDashboard = { todaySources: number; pendingCandidates: number; todayAnalyzed: number; strongRecommend: number; recommend: number; watch: number; reject: number; averageMarginBps: number; runningBatches: CandidateAnalysisBatch[]; pausedBatches: number; failedBatches: number; marketCoverageBps: number; performanceUpdatedAt?: string; operationalSummary: string };
