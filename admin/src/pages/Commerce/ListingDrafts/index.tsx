...
export default function ListingDraftsPage() {
  ...
  return <TmPageContainer title="铺货中心" subTitle="本阶段仅编辑草稿，不调用闲鱼或淘宝发布接口">
    <ProTable<ListingDraft> key="listing-drafts-table" rowKey="id" actionRef={actionRef} columns={columns} request={async (p) => { const r = await fetchListingDrafts(p); return { data: r.list, total: r.pagination.total, success: true }; }} />
